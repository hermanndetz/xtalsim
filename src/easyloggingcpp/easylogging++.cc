...
base::type::string_t DateTime::formatTime(unsigned long long time, base::TimestampUnit timestampUnit) {
  ...
  for (base::type::EnumType i = start; i < base::consts::kTimeFormatsCount - 1; ++i) {
    ...
    if ( (FP_ZERO == std::fpclassify(base::consts::kTimeFormats[i].value - 1000.0f) ) && time / 1000.0f < 1.9f) {
      break;
    }
...
Writer& Writer::construct(int count, const std::string loggerIds, ...) {
  if (ELPP->hasFlag(LoggingFlag::MultiLoggerSupport)) {
    va_list loggersList;
    va_start(loggersList, loggerIds);
    const char* id = loggerIds.c_str();
...
// Same as above just with name as char*
