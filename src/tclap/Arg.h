...
inline void Arg::addToList( std::list<Arg*>& argList ) const
{
	...
	if (argList.size() == 0){
 	...
 	for (auto it=argList.begin(); it != argList.end(); it++){
 		shortName = (*it)->shortID();
 		introEnd = shortName.find_last_of("-");
 		shortName = shortName.substr(introEnd+1);
 		if ( ( (_flag != "") && (_flag < shortName) ) || (_name < shortName) ){
 			argList.insert(it, const_cast<Arg*>(this));
			return;
		}
	}
